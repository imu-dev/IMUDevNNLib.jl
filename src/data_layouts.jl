...
!!! tip "Example"
    ...
    day @1Hz. The data extracted from this device represented in a `Matrix` form
    with dimensions `3×86_400`, with each column representing a single
    observation of the 3d-acceleration would correspond to
...
!!! tip "Example"
    ...
    recording for one day @1Hz. The data extracted from these two devices
    represented in a `Vector{Matrix}` form with length `86_400` and each element
    of this vector having dimensions `3×2` (i.e. it would be two vectors of
    3d-accelerations horizontally concatenated together into a `3×2` `Matrix`)
    would correspond to `TimeseriesLayout`.
"""
...
"""
Data stored in the `SingleArrayLayout` is an N-dimensional array with the
last, N-th dimension indexing over samples.

!!! note
    For a particular case of `SingleArrayLayout` intended to respresent time
...
"""
struct SingleArrayLayout <: DataLayout end

"""
    write_header!(io::IOStream, ::SingleArrayLayout, dims)

Utility function for writing the header of a binary file that stores data in
`SingleArrayLayout`. `dims` is a tuple of integers representing the dimensions
of the data.
"""
function write_header!(io::IOStream, ::SingleArrayLayout, dims)
    length(dims) >= 3 ||
        throw(ArgumentError("`SingleArrayLayout` must be given by at least three dimensional arrays"))
    write(io, length(dims) - 2)
    return write(io, [dims...])
end

"""
    read_header(io::IOStream, ::SingleArrayLayout)

Utility function for reading the header of a binary file that stores data in
`SingleArrayLayout`.
"""
function read_header(io::IOStream, ::SingleArrayLayout)
...
state_dim(::SingleArrayLayout, data::AbstractArray) = size(data)[1:(ndims(data) - 2)]
...
"""
    ...
    num_timepoints(layout::SingleArrayLayout, data::AbstractArray)
...
num_timepoints(::SingleArrayLayout, data::AbstractArray) = size(data, ndims(data) - 1)
...
function Base.reshape(data::AbstractArray,
                      ::Pair{SingleArrayLayout,TimeseriesLayout})
...
function Base.reshape(data::Vector{<:AbstractArray},
                      ::Pair{TimeseriesLayout,SingleArrayLayout})
...
"""
    dimsofreshape(data::Vector{<:BatchDelimitedArray},
                  from_to::Pair{TimeseriesLayout,SingleArrayLayout})
                
Compute the size of the `Array` that would be needed to reshape `data` stored in
`TimeseriesLayout()` into a format following `SingleArrayLayout()`.
